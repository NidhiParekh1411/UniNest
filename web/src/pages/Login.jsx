...
// merged on the other. Both render the same centred card — the segmented
// switch at the top is the only chrome that distinguishes them, so arriving at
// the wrong door costs one tap rather than a wasted password attempt.
const COPY = {
  student: {
  ...
  return (
    <div className="auth">
      <div className="auth-shell">
                ...
                {accounts.map((a) => (
                  <button key={a.email} type="button" className="demo-account" onClick={() => useAccount(a)} disabled={busy}>
                    ...
                    <span className="grow" style={{ minWidth: 0 }}>
                      <span className="demo-account-name" style={{ display: 'block' }}>{a.name}</span>
        ...
        <p className="dim center" style={{ fontSize: 'var(--fs-xs)', maxWidth: '52ch', marginInline: 'auto' }}>
